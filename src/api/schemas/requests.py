...
class QuestionRequest(BaseModel):
    ...
    question: str = Field(
        ..., min_length=3, max_length=1000, description="Текст юридического вопроса"
    )
    user_id: str = Field(
        ..., min_length=1, max_length=255, description="Идентификатор пользователя"
    )
    conversation_id: Optional[str] = Field(
        None, description="ID существующего диалога (если None - создаётся новый)"
    )
...
class CreateConversationRequest(BaseModel):
    ...
    title: Optional[str] = Field(
        None, max_length=500, description="Название диалога (опционально)"
    )
...
class UpdateConversationRequest(BaseModel):
    ...
    title: Optional[str] = Field(
        None, max_length=500, description="Новое название диалога"
    )
